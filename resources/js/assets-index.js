...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    const perPageSelect = document.getElementById('assetsPerPage');
            ...
            <div class="company-status-cell">
                <div class="form-check form-switch company-status-switch company-status-switch--solo mb-0">
                    <input
                        ...
                        ${isActive ? 'checked' : ''}
                        aria-label="Toggle asset type status"
                    >
                </div>
            </div>
        `;
    ...
    const setStatusToggle = (assetTypeId, status) => {
        const toggle = tableBody.querySelector(`[data-status-toggle="${assetTypeId}"]`);
    ...
    bindPagination(paginationList, loadAssets);
    ...
    tableBody.addEventListener('change', async (event) => {
        ...
        try {
            ...
            setStatusToggle(assetTypeId, data.data.asset_type.status);
            showAlert(data.message || 'Asset status updated successfully.');
        } catch (error) {
            toggle.checked = previousChecked;
            setStatusToggle(assetTypeId, previousChecked ? 'active' : 'inactive');
