    ...
    const renderRow = (holiday, index, pagination) => {
        ...
        return `
            <tr class="companies-data-row">
                ...
                ${canManage ? `<td>${renderStatusPill(holiday.status)}</td>${actionsCell}` : ''}
            </tr>
        `;
    };
    ...
    const loadHolidays = async (page = 1) => {
        ...
        try {
            ...
            if (holidays.length === 0) {
                tableBody.innerHTML = `<tr><td colspan="${columnCount}" class="text-center text-muted py-5">No holidays found.</td></tr>`;
            } else {
            ...
            tableBody.innerHTML = `<tr><td colspan="${columnCount}" class="text-center text-danger py-5">${getErrorMessage(error)}</td></tr>`;
        }
    };
