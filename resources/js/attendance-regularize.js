...
const formatEligiblePunchMeta = (item) => {
    ...
    const updateEligibleSelectionUi = () => {
        ...
        if (openRegularizeRequestBtn) {
            const limitReached = regularizationPolicy?.remaining_requests === 0;
            openRegularizeRequestBtn.disabled = count === 0 || limitReached;
            openRegularizeRequestBtn.textContent = limitReached
    ...
    const updateOriginalTimesUi = () => {
        // Per-day times are shown inside each selected date row.
    };
    ...
    const updateSelectedDatesUi = () => {
                ...
                return `
                    <li class="regularize-dates-list-item regularize-dates-list-item--expandable${expanded ? ' is-expanded' : ''}" data-regularize-date-row="${date}">
                        ...
                        <div class="regularize-dates-list-item-fields">
                            <div class="row g-2">
                                <div class="col-sm-6">
                                    <label class="form-label small mb-1" for="regularize_in_${date}">New login</label>
                                    <input
                                        type="time"
                                ...
                                <div class="col-sm-6">
                                    <label class="form-label small mb-1" for="regularize_out_${date}">New logout</label>
                                    <input
                                        type="time"
                                        class="form-control form-control-sm"
        ...
        if (regularizeSubmitBtn) {
            ...
            const limitReached = regularizationPolicy?.remaining_requests === 0;
            regularizeSubmitBtn.disabled = count === 0 || limitReached;
