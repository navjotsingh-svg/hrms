...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    const renderRow = (documentType, index, pagination) => {
        ...
        const requiredBadge = documentType.is_required
            ...
            : '<span class="badge text-bg-light text-muted">Optional</span>';
        const uploadBadge = documentType.allow_multiple
            ...
            <tr class="companies-data-row">
                ...
                <td>
                    <div class="companies-company-info">
                        <span class="companies-company-name">${documentType.name}</span>
                        ${documentType.description ? `<span class="companies-company-meta">${documentType.description}</span>` : ''}
                    </div>
                </td>
                <td>${documentType.code || '—'}</td>
                <td>${requiredBadge}</td>
                <td>${uploadBadge}</td>
                <td>${renderStatusToggle(documentType)}</td>
                <td class="companies-td-actions">
                    <div class="table-action-group">
                        <a href="${editUrl}" class="table-action-btn table-action-btn--edit" title="Edit" aria-label="Edit ${documentType.name}">
                            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" viewBox="0 0 16 16"><path d="M12.146.146a.5.5 0 0 1 .708 0l3 3a.5.5 0 0 1 0 .708l-10 10a.5.5 0 0 1-.168.11l-5 2a.5.5 0 0 1-.65-.65l2-5a.5.5 0 0 1 .11-.168zM11.207 2.5 13.5 4.793 14.793 3.5 12.5 1.207zm1.586 3L10.5 3.207 4 9.707V10h.5a.5.5 0 0 1 .5.5v.5h.5a.5.5 0 0 1 .5.5v.5h.293zm-9.761 5.175-.106.106-1.528 3.821 3.821-1.528.106-.106A.5.5 0 0 1 5 12.5V12h-.5a.5.5 0 0 1-.5-.5V11h-.5a.5.5 0 0 1-.468-.325"/></svg>
                        </a>
                        <button type="button" class="table-action-btn table-action-btn--delete" title="Delete" aria-label="Delete ${documentType.name}" data-delete-document="${documentType.id}">
    ...
    const loadDocuments = async (page = 1) => {
        ...
        try {
            ...
            const documentTypes = data.data.document_types || [];
            const pagination = data.data.pagination;

            if (documentTypes.length === 0) {
                tableBody.innerHTML = '<tr><td colspan="7" class="text-center text-muted py-5">No document types found.</td></tr>';
            } else {
                tableBody.innerHTML = documentTypes.map((documentType, index) => renderRow(documentType, index, pagination)).join('');
            }
    ...
    filterSearch?.addEventListener('input', () => {
        ...
        searchTimeout = setTimeout(() => loadDocuments(1), 400);
    });

    filterStatus?.addEventListener('change', () => loadDocuments(1));
    ...
    tableBody.addEventListener('change', async (event) => {
        ...
        const documentTypeId = toggle.dataset.statusToggle;
    ...
    tableBody.addEventListener('click', async (event) => {
        ...
        try {
            const { data } = await api.delete(`/document-types/${button.dataset.deleteDocument}`);
            showAlert(data.message || 'Document type deleted successfully.');
