...
document.addEventListener('DOMContentLoaded', async () => {
            ...
            <div class="company-status-cell">
                <div class="form-check form-switch company-status-switch mb-0">
                    ...
                    <label
                        ...
                        for="${switchId}"
                        data-status-label="${department.id}"
                    >
    ...
    const setStatusLabel = (departmentId, status) => {
        const label = tableBody.querySelector(`[data-status-label="${departmentId}"]`);
    ...
    const renderPagination = (pagination) => {
        ...
        if (!pagination?.total) {
            paginationInfo.textContent = 'No departments found';
            paginationList.innerHTML = '';
            return;
        }

        paginationInfo.textContent = `Showing ${pagination.from || 0} to ${pagination.to || 0} of ${pagination.total} departments`;
    ...
    paginationList?.addEventListener('click', (event) => {
        ...
        loadDepartments(Number(button.dataset.page));
    });
    ...
    tableBody.addEventListener('change', async (event) => {
        ...
        try {
            ...
            setStatusLabel(departmentId, data.data.department.status);
            showAlert(data.message || 'Department status updated successfully.');
        } catch (error) {
            toggle.checked = previousChecked;
            setStatusLabel(departmentId, previousChecked ? 'active' : 'inactive');
