...
class AttendanceRegularizationService
{
    ...
    public function eligibleDatesForUser(User $user, ?int $employeeId = null): array
    {
        $employee = $this->resolveTargetEmployee($user, $employeeId);
        $employee->loadMissing('shift');

        $rangeEnd = now()->copy()->subDay();
        ...
        $rangeStart = Carbon::parse($trackingStart);
        ...
        for ($date = $rangeStart->copy(); $date->lte($rangeEnd); $date->addDay()) {
            ...
            $pendingRequest = $this->pendingForEmployeeDate($employee, $dateString);

            if ($pendingRequest) {
                $dates[] = $this->formatEligibleDate(
                    $dateString,
                    $date,
                    $dayMeta,
                    $employee,
            ...
            if (! $this->canRequestForDate($user, $employee, $dateString)) {
                continue;
            }

            $dates[] = $this->formatEligibleDate($dateString, $date, $dayMeta, $employee, false);
        }
    ...
    private function assertCanRequestForDate(User $user, Employee $employee, string $date): void
    {
        ...
        if ($this->attendancePolicyService->isWeeklyOff(
            $date,
            $this->attendancePolicyService->weeklyOffWeekdays($employee->company_id),
        )) {
            throw ValidationException::withMessages([
